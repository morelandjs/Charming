...
{
  paraRdr = paraRdr_in; // where all parameters are
...
void MakeDensity::generate_profile_ebe(int nevent)
{
  ...
  char file_bctable[] = "%s/bctable_event_%d.dat";

  // entropy profile:
  char file1_4col[] = "%s/sd_event_%d_4col.dat";
  char file1_block[] = "%s/sd_event_%d_block.dat";
  ...
  // energy profile:
  char file2_4col[] = "%s/ed_event_%d_4col.dat";
  char file2_block[] = "%s/ed_event_%d_block.dat";
  ...
  int bc_table = paraRdr->getVal("bc_table");
  ...
  while (event<=nevent)
  {
    ...
    while (binary==0 || mc->CentralityCut()==0)
    {
      ...
      // handles binary collision table output
      binary = mc->getBinaryCollision();
      sprintf(buffer,file_bctable,cpath,event);
      if(bc_table==1) mc->dumpBinaryTable(buffer); 
    ...
    for(int iy=0;iy<binRapidity;iy++) {
      ...
      if (use_sd)
      {
        ...
        if (use_4col)
          {
            sprintf(buffer,file1_4col,cpath,event);
            dumpDensity4Col(buffer, dens1, iy);
          }
        if (use_block)
          {
            sprintf(buffer,file1_block,cpath,event);
      ...
      if (use_ed)
      {
        ...
        if (use_4col)
          {
            sprintf(buffer,file2_4col,cpath,event);
            dumpDensity4Col(buffer, dens2, iy);
          }
        if (use_block)
          {
            sprintf(buffer,file2_block,cpath,event);
...
void MakeDensity::generate_profile_average(int nevent)
{
  ...
  // binary collision table
  char file_bctable[] = "%s/bctable_order_%d.dat";

  // entropy profile:
  char file1_4col[] = "%s/sdAvg_order_%d_4col.dat";
  char file1_block[] = "%s/sdAvg_order_%d_block.dat";
  ...
  // energy profile:
  char file2_4col[] = "%s/edAvg_order_%d_4col.dat";
  char file2_block[] = "%s/edAvg_order_%d_block.dat";
  ...
  while (event<=nevent)
  {
    ...
    while (binary==0 || mc->CentralityCut()==0)
    {
      ...
      // outputs binary collision table
      binary = mc->getBinaryCollision();
      char buffer[200];
      sprintf(buffer,file_bctable,cpath,event);
      if(bc_table==1) mc->dumpBinaryTable(buffer);
  ...
  for (int iorder=0; iorder<number_of_orders; iorder++)
  {
    ...
    for (int iy=0; iy<binRapidity; iy++)
    {
      ...
      if (use_sd)
      {
        if (use_4col)
          {
            sprintf(buffer, file1_4col, cpath, order);
            dumpDensity4Col(buffer, dens1[iorder], iy);
          }
        if (use_block)
          {
            sprintf(buffer, file1_block, cpath, order);
      ...
      if (use_ed)
      {
        if (use_4col)
          {
            sprintf(buffer, file2_4col, cpath, order);
            dumpDensity4Col(buffer, dens2[iorder], iy);
          }
        if (use_block)
          {
            sprintf(buffer, file2_block, cpath, order);
...
// Npmin, Npmax: cut on N_part
{
  ...
  // entropy profile:
  char file1[] = "%s/sn_ecc_eccp_%d.dat";
  double *** dens1  = new double** [binRapidity];
  for(int iy=0;iy<binRapidity;iy++) {
    dens1[iy] =  new double* [Maxx]();
    for(int i=0;i<Maxx;i++) {
  ...
  // energy profile:
  char file2[] = "%s/en_ecc_eccp_%d.dat";
  double *** dens2  = new double** [binRapidity];
  for(int iy=0;iy<binRapidity;iy++) {
    dens2[iy] =  new double* [Maxx]();
    for(int i=0;i<Maxx;i++) {
  ...
  while (event<=nevent)
  {
    ...
    for(int iy=0;iy<binRapidity;iy++) {
      ...
      {
        setSd(dens1, iy);
        dumpEccentricities(file1, dens1, iy, from_order, to_order, Npart, b);
      }
      // energy second
      if (paraRdr->getVal("use_ed"))
      {
        setEd(dens2, iy);
        dumpEccentricities(file2, dens2, iy, from_order, to_order, Npart, b);
      }
    }
...
//********************************************************************
void MakeDensity::dumpEccentricities(char* base_filename, double*** density, const int iy, int from_order, int to_order, double Npart_current, double b)
// calculate and output eccentricities
{
    ...
    for (order=from_order; order<=to_order; order++)
    {
        ...
        of  << setprecision(8) << setw(20) <<  mom_real[order]
            ...
            << setprecision(3) << setw(12) <<  Npart_current
